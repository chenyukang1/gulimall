...
如果电脑配置不高，不建议用虚拟机部署。准备 2 台 2c4g 的服务器，一台给 ES、Kibana、RabbitMQ，其中 ES 和 Kibana 必须限制最大堆内存；另一台给 Redis、MySQL、Nacos

在做域名搭建的时候，如果一开始把 Nginx 放在云服务器，然后在本地开发会比较麻烦，要做内网穿透，建议先把 Nginx 放在本地，等到基本功能开发完成，可以做压测的时候，再全部上云

## 环境搭建

所有环境通过 docker 部署，分配独立的桥接网络，参考 gulimall-devops 模块
